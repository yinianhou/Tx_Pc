...
namespace Tx.Bussiness.Service
{
    /// <summary>
    /// 这是BLL基类，就是为了泛型重用
    /// </summary>
         ...
         public enum EntityState
    {
        ...
        public void Delete<T>(IEnumerable<T> tList) where T : class
        {
            ...
            //this.Context.Set<T>().Remove(tList);
            this.Commit();
        }
        #endregion
        ...
        public UserSession CurrentUser
        {
            ...
            {
                _currentUser = null;
                if (!string.IsNullOrEmpty(CurrentRequest.UserId))
                {
                    _currentUser = new UserSession();
                    _currentUser.UserId = CurrentRequest.UserId;
                    _currentUser.UserName = CurrentRequest.UserName;
                }
                return _currentUser;
            }
        }
        private UserIdentity _currentRequest = new UserIdentity();
        public UserIdentity CurrentRequest
        {
            get
            {
                string o = CallContext.LogicalGetData(DefaultAttribute.USER_INDETITY) as string;
                _currentRequest.UserId = o;
                o = CallContext.LogicalGetData(DefaultAttribute.USER_IP) as string;
                _currentRequest.IP = o;
                object name = CallContext.LogicalGetData(DefaultAttribute.USER_NAME) as string;
                _currentRequest.UserName = name?.ToString();
